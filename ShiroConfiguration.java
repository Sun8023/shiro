...
import com.jianpiao.core.commons.utils.StringUtils;
import com.jianpiao.util.RedisKeyPrefix;
...
public class ShiroConfiguration {
    ...
    /**
     * https配置，构造sslFilter拦截器时，默认端口为443；
     *
     * @return
     */
    @Bean("sslFilter")
    ...
    @Bean("shiroFilter")
    public AbstractShiroFilter shiroFilter(@Qualifier("securityManager") SecurityManager manager
            , @Qualifier("corsFilter") OncePerRequestFilter corsFilter) throws Exception {
    ...
    public SimpleCookie simpleCookie() {
        SimpleCookie simpleCookie = new SimpleCookie("JP_SESSIONSID");
    ...
    @Bean
    public CustomDefaultFilterChainManager customDefaultFilterChainManager(@Value("${anon}") String anonUrls
            , @Value("${authc}") String authcUrls, @Value("${ssl}") String sslUrls
            , @Qualifier("sslFilter") SslFilter sslFilter) {
        ...
        if (StringUtils.isNotBlank(sslUrls)) {
            String[] sslUrlArr = sslUrls.split(",");
            for (String sslUrl : sslUrlArr) {
                filterChainDefinitionMap.put(sslUrl, "ssl");
            }
        }
        //ssl
        Map<String, Filter> customerFilter = new HashMap<>();
        customerFilter.put("ssl", sslFilter);

        customDefaultFilterChainManager.setCustomFilters(customerFilter);
